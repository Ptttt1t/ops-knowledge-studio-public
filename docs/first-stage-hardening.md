# 第一阶段可信度加固

初次完成：2026-07-17；字段指针与审计加固：2026-07-22；回答协议兼容加固：2026-08-13

## 完成内容

### 1. 检索门槛与严格拒答

- 中文单字停用词和常见英文停用词不再参与召回。
- 检索结果同时满足最低相关度和查询词覆盖率才可进入可信回答。
- 默认配置为 `KNOWLEDGE_MIN_SCORE=10`、`KNOWLEDGE_MIN_COVERAGE=0.15`。
- 没有强相关 APPROVED 卡片时直接返回 `no_relevant_approved_knowledge`，不调用回答模型。

### 2. 逐结论结构化引用

- 模型不再生成结论正文，而是输出 `card_id + support_field + support_index` 字段指针。
- `card_id` 必须来自本次实际检索到的 APPROVED 卡片，字段和类别必须匹配，下标必须有效。
- 结论正文由程序从已审核卡片的结构化字段中确定性取值；模型自由 `text`、旧式 `card_ids` 和越界字段会被拒绝。
- Markdown 答案、`[K编号]` 和来源列表由同一份程序校验后的结构生成。
- “知识不足”只能指向实际为空的卡片字段，不能借此写入自由建议。
- 模型返回空 claims 时按证据不足拒答。
- 提示协议要求优先不超过 12 条 claims，平台在稳定去重后执行 30 条硬上限，并保留 120 条原始输出安全上限。
- 标量字段的 `support_index: 0` 或 `"0"` 会归一化为 `null`；布尔值、其他数字和越界数组下标仍拒绝。
- 首次输出违反协议时允许一次有界纠错重试；直接查询与只读 Agent 复用相同路径，重试次数进入响应和轨迹。

### 3. 原文证据回定位

- 先尝试逐字匹配，再处理空白、Unicode 标点和大小写规范化。
- 对 Markdown 展示指令采用保守的首尾锚点匹配。
- 最终存储的引文始终是程序从来源分片截取的连续原文。
- 证据位置缩小为真实字符区间，并记录 `exact / normalized / anchored` 匹配方式。
- 无法回定位的卡片保持 DRAFT，不能只靠质量分进入待审队列。

### 4. 分类质量评分

- `procedure` 继续检查步骤、风险、回退和验证。
- `risk` 重点检查风险事实，不再强制要求回退步骤。
- `case` 重点检查事件影响。
- `constraint` 不再被操作步骤字段误罚。
- `compatibility` 检查版本或适用范围。
- `rollback` 重点检查回退和验证字段。

### 5. 批内去重与失败回滚

- 同一文档各分片抽取出的卡片先按规范化结构化字段生成批次指纹，重复项不会再次比较或入库。
- 导入结果返回 `batch_duplicates_skipped`；重复项中优先保留证据可定位、质量分更高的候选，并在轨迹中记录取舍。
- 文件上传在解析或知识导入失败后删除本次新建文件，避免无效来源永久残留。
- 版本替代只允许 `DRAFT/PENDING_REVIEW` 新卡替代 `APPROVED` 旧卡，并为新旧两张卡片同时写审计。

## 现有知识库迁移结果

执行：

```powershell
python run.py regrade
```

结果：

- 处理卡片：73
- 成功回定位原文：53
- 当前平均质量分：82.1
- 当前状态：DRAFT 22 / PENDING_REVIEW 46 / APPROVED 5
- 全过程不调用大模型，不会改变 APPROVED、REJECTED、SUPERSEDED 的人工治理结论

## 验收结果

- 2026-07-22 离线单元与集成测试：23/23 通过。
- 四个正向公开语料检索：4/4 目标卡 Top 1。
- 旧版自由文本 claims 曾完成三次真实 DeepSeek 验证；字段指针协议本轮只完成 Fake Client 离线回归，尚未使用真实 API 复测。
- 负向问题“HTTP 503 和 Retry-After 表示什么”：返回零命中，不调用回答模型。
- 测试脚本退出码：0；公开版不包含带本机路径和运行数据的 `artifacts/` 结果文件。

## 配置

```dotenv
KNOWLEDGE_TOP_K=6
KNOWLEDGE_MIN_SCORE=10
KNOWLEDGE_MIN_COVERAGE=0.15
```

阈值应继续通过固定评测集调优，不能把当前默认值理解为所有数据规模下的最终参数。
